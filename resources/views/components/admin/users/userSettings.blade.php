...
<div class="modal fade"
     ...
     tabindex="-1"
     role="dialog"
     aria-labelledby="gallery-modalLabel"
     aria-hidden="true">
        ...
        <div class="modal-content">
            <div class="modal-header modal-colored-header bg-primary">
                <h5 class="modal-title"
                    id="gallery-modalLabel">Media Library</h5>
                <button type="button"
                        class="close"
                        data-dismiss="modal"
                        aria-label="Close">
...
<div class="row">
    <div class="col-xl-9 col-lg-8 col-md-12"><!-- col @s -->
        <form id="edit-user-form"
              action="/dashboard/users/{{ $user->slug }}"
              method="POST"
                ...
                <div class="card-body"><!-- card-body @s -->
                    ...
                    <div class="form-group mb-0">
                        ...
                        <textarea name="profil"
                                  ...
                                  rows="5">{{ old("profil", $user->profil) }}</textarea>
                        @error('profil')
                        <span class="invalid-feedback"
                              role="alert">
								<strong>{{ $message }}</strong>
                        ...
                        <div class="dropdown-menu  dropdown-menu-right py-0">
                            ...
                            <div class="btn-group dropleft dropleft-hover w-100">
                                ...
                                <div class="dropdown-menu py-0"
                                    ...
                                    <div class="dropdown-divider my-0"></div>
                                    <a id="show-password"
                                       class="dropdown-item cursor-pointer py-2">Εμφάνιση</a>
                                </div>
                            </div>
                        </div>
                ...
                <div class="form-group mb-0">
                    ...
                    <select form="edit-user-form"
                        ...
                        @if ($userRole === "super-admin")
                            <option value="super-admin" {{ old("roles", $userRole ) === "super-admin" ? "selected" : ""}}>
                                Super Admin
                            </option>
                        @endif
                        <option value="admin" {{ old("roles", $userRole ) === "admin" ? "selected" : ""}}>Admin</option>
                        <option value="instructor" {{ old("roles", $userRole ) === "instructor" ? "selected" : ""}}>
                            Εισηγητής
                        </option>
                        <option value="student" {{ old("roles", $userRole ) === "student" ? "selected" : ""}}>Μαθητής
                        </option>
                        <option value="partner" {{ old("roles", $userRole ) === "partner" ? "selected" : ""}}>Partner
                        </option>
                    </select>
                </div>
...
@endif timeline-icon"></i>
                            <div class="timeline-item-info">
                                <span href="#"
                                      class="{{ $activity->log_name === "login" ? "text-success" : "text-primary"}} font-weight-bold mb-1 d-block">{{ ucfirst($activity->log_name) }}</span>
                                <p class="mb-0 pb-2">
