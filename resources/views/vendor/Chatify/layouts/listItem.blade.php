        ...
        <tr data-action="0">
            ...
            <td>
                <div class="avatar av-m"
                     style="background-color: #d9efff; text-align: center;">
            ...
            {{-- center side --}}
            <td>
                <p data-id="{{ 'user_'.Auth::user()->id }}">Αποθηκευμένα μηνύματα <span>Εσύ</span></p>
                <span>Αποθηκεύστε τα μηνύματα κρυφά</span>
            </td>
        </tr>
    </table>
@endif
        ...
        <tr data-action="0">
            ...
            <td>
                <p data-id="{{ $type.'_'.$user->id }}">
                    {{ strlen($user->fullName) > 12 ? trim(substr($user->fullName,0,80)).'..' : $user->fullName }}
                    <span>{{ $lastMessage->created_at->diffForHumans() }}</span></p>
                <span>
            {{-- Last Message user indicator --}}
                    {!!
                        $lastMessage->from_id == Auth::user()->id
                        ? '<span class="lastMessageIndicator">Εσύ :</span>'
                        : ''
                    !!}
                    {{-- Last message body --}}
                    ...
                    @else
                        <span class="fas fa-file"></span> Συνημμένα
                    @endif
        </span>
...
@if($get == 'search_item')
    <table class="messenger-list-item"
        ...
        <tr data-action="0">
            ...
            <td>
                <p data-id="{{ $type.'_'.$user->id }}">
                {{ strlen($user->name) > 50 ? trim(substr($user->name,0,50)).'..' : $user->name }}
            </td>
